...
using GN.Library.Odoo.Internal.Abstract;
...
namespace GN.Library.Odoo.Internal.Concrete
{
	public class RpcConnection
	{
		...
		public int[] SearchEx(string model, object[] filter, int offset = 0, int? limit = null, string order = "")
		{
			if (limit == null)
			{
				return _objectRpc.search(_rpcConnectionSchema.DbName, _rpcConnectionSchema.UserId, _rpcConnectionSchema.DbPassword, model, "search", filter);
			}
			return _objectRpc.search(_rpcConnectionSchema.DbName, _rpcConnectionSchema.UserId, _rpcConnectionSchema.DbPassword, model, "search", filter, offset, (int)limit, order);
		}

		public object[] SearchAndReadEx(string model, object[] filter, object[] fields, int offset = 0, int? limit = null, string order = null)
		{
			if (limit == null)
			{
			...
			return _objectRpc.search_read(_rpcConnectionSchema.DbName, _rpcConnectionSchema.UserId, _rpcConnectionSchema.DbPassword, model, "search_read", filter, fields, offset, (int)limit, order ?? "");
		}
		...
		public bool Execute_Method(string model, string methodName, int[] ids)
		{
			return _objectRpc.ExecuteMethod(_rpcConnectionSchema.DbName, _rpcConnectionSchema.UserId, _rpcConnectionSchema.DbPassword, model, methodName, ids);
		}
	}
}
