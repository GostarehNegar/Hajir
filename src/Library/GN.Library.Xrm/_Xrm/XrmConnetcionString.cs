...
namespace GN.Library.Xrm
{
	...
	public class XrmConnectionString
	{
		...
		{
			return $"Url={url ?? Url}; Username={userName ?? UserName}; Domain={domainName ?? DomainName}; Password={password ?? Password}; Data Source={this.DataSource}; Initial Catalog={this.InitialCatalog}; AuthType={AuthType}";
		}
		/// <summary>
		...
		public XrmConnectionString Reset(string connectionString)
		{
			...
			if (!string.IsNullOrWhiteSpace(connectionString))
			{
				...
				this.DataSource = getValue("Data Source");
				this.InitialCatalog = getValue("Initial Catalog");
				this.Password = getValue("password")?.Trim();
				this.AuthType = getValue("AuthType")?.Trim();
			}
		...
		public XrmConnectionString(string connectionString)
		{
			Reset(connectionString ?? "");
		}

		public XrmConnectionString(IConfiguration settings)
		{
		...
		{
			return new XrmConnectionString("").Reset(this.BuildConnetionString());
		}
	}
