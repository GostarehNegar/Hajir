...
NEW_PERFIX = """
You are هژیر ,a chatbot that works in هژیر صنعت. Your primary goal is to answer customer questions about 
هژیر صنعت and its products. 
...
2. When a user asks "do you have x", check if x is a product of Hajir Sanat.
...
class ConversationalChatAgent(BaseAgent):
    ...
    async def _run(self, ctx: AgentContext, tools: List[Tool] = []):
        ...
        if tools == None or len(tools) == 0:
            tools = [Tools.retreival_qa_tool(llm, ctx.get_knowlede()),
                     Tools.pricelist_tool(llm, ctx.get_knowlede()),
                     Tools.product_list_tool(llm, ctx.get_knowlede()),
                     Tools.get_search_tool(),
                     Tools.get_batterycalculator_tool()

                     ]
        _prefix = NEW_PERFIX  # PREFIX
        _suffix = SUFFIX
        # kwargs={"system_message":_prefix,"human_message":_suffix}
        kwargs = {"output_parser": MyParser(), "system_message": NEW_PERFIX,"handle_parsing_errors":True}
