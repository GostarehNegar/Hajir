...
namespace Hajir.Crm.Integration
{
    ...
    internal class IntegrationQueue
    {
        ...
        public void Upsert(string id, string logicalName)
        {
            ...
            repo.Upsert(new IntegrationEntry { Id = id, LogicalName = logicalName });
    ...
    public class IntegrationQueueEx : LiteDatabaseEx, IIntegrationQueue
    {
        ...
        {
            var result = await GetCollection<IntegrationEntry>(false, default);
        ...
        {
            var result = 0;
            using (var col = await GetCollection())
            {
                foreach (var item in items)
                {
        ...
        {
            using (var c = await GetCollection())
            {
